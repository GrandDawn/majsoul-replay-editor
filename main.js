...
function calcfan(tls,seat,zimo){
  ...
  function calc(){
    ...
    function calc0(tingpaifu){
      ...
      if(typecnt[tiletoint(((seat-chang+playercnt)%playercnt+1).toString()+"z")][1]==2)ans.fans.push({'val':1,'id':10});//门风 
...
function addBaBei(seat){
  let moqie;
  if(playertiles[player][playertiles[player].length-1]=="4z")moqie=true;
  else moqie=false;
  if(doracnt.lsttype==1){
    doracnt.lsttype=0;
    doracnt.cnt++;
...
function endLiuJu(seat,tiles,type){
  actions.push({
    name:"RecordLiuJu",
    data:{
      'seat':seat,
...
function qie(player,kind,is_liqi,var1){
  let is_wliqi=false;
...
function huangpailiuju(){
  ...
  let ret2=[];
  for(let i=0;i<playercnt;i++){
    console.log(i);console.log(paihe[i].liujumanguan);
    if(!paihe[i].liujumanguan)continue;
    console.log(i);
    liujumanguan=true;
    let score=0;
    playertiles[i].sort(cmp);
	...
	for(let j=0;j<playercnt;j++){
      scores[j]=scores[j]+delta_scores[j];
      delta_scores[j]=0;
    }
  }
  if(liujumanguan){
  ...
  if(liujumanguan==false&&tingcnt!=0&&tingcnt!=playercnt){
    for(let i=0;i<playercnt;i++){
      if(ret[i].tingpai==true)delta_scores[i]+=(playercnt-1)*1000/tingcnt;
      else delta_scores[i]-=(playercnt-1)*1000/(playercnt-tingcnt);
    }
  }
...
function gameend(){
  scores[ju]+=liqibang;
  function cmp2(x,y){
    return y.part_point_1-x.part_point_1;
  }
  players=[];
...
function randompaishan(paishan){
...
firstneededscores=5000+scores[0];
//第一局（作弊） 
